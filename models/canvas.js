...
function computeScoreAndBadges(studentID, courseID, callback) {
  // Return score and badges
  mongo.getAllData(courseID, function(mongo_data) {
      ...
      for (lucky_bulldog of mongo_data.lucky_bulldogs) {
        ...
        if (lucky_bulldog.awarded_ids.length > 0) {
          if (lucky_bulldog.awarded_ids.includes(studentID)) {
            ...
            (d.getTime() - Date.parse(lucky_bulldog.time)) / (1000 * 60) <
            1
          ) {
            totalPoints += parseInt(lucky_bulldog_points);
            ...
            mongo.updateData(
              courseID,
              "lucky_bulldogs",
              { _id: parseInt(lucky_bulldog._id) },
              { awarded_ids: lucky_bulldog.awarded_ids },
              function(err, result) {}
            );
          }
        } else if (
    ...
    getRequest(assignment_user_url(studentID, courseID), studentID, function(
      err,
      data
    ) {
      if (err) {
        ...
        for (var i = 0; i < mongo_data.dailies.length; i++) {
          var daily_object = data.find(
            daily => daily.assignment_id == mongo_data.dailies[i].assignment_id
          );
          if (daily_object) {
        ...
        for (var i = 0; i < mongo_data.modules.length; i++) {
          if (mongo_data.modules[i].open == "true") {
            ...
            var practice_object = data.find(
              assignment =>
                assignment.assignment_id == mongo_data.modules[i].practice_link
            );
            if (practice_object) {
              var practice_grade = parseFloat(practice_object.grade);
              if (
                practice_grade >=
                parseFloat(mongo_data.modules[i].practice_cutoff)
              ) {
                practice_proficient += 1;

                //Process Practice Early Bird Badge
                if (
                  mongo_data.modules[i].leaderboard.practice_early_bird == ""
                ) {
                  mongo_data.modules[
                    i
                  ].leaderboard.practice_early_bird = studentID.toString();
                  awardBadge(26);
                } else {
                  if (
                    mongo_data.modules[i].leaderboard.practice_early_bird ==
                ...
                if (
                  mongo_data.modules[i].leaderboard.practice_leaderboard.find(
                    placement => placement.student_id == studentID
                  )
                ) {
                  //user is already on leaderboard
                  awardBadge(20);
                  user_index = mongo_data.modules[
                    i
                  ].leaderboard.practice_leaderboard.findIndex(
                    placement => placement.student_id == studentID
                  );
                  mongo_data.modules[i].leaderboard.practice_leaderboard[
                    user_index
                  ] = {
                  ...
                  mongo_data.modules[
                    i
                  ].leaderboard.practice_leaderboard = mongo_data.modules[
                    i
                  ].leaderboard.practice_leaderboard.sort(
                  ...
                  if (
                    ...
                    if (
                      ...
                      mongo_data.modules[
                        i
                      ].leaderboard.practice_leaderboard.pop();
                      mongo_data.modules[
                        i
                      ].leaderboard.practice_leaderboard.push({
                        student_id: studentID.toString(),
                        score: practice_grade
                      });
                      awardBadge(20);
            ...
            //quizzes attempted
            var quiz_object = data.find(
              assignment =>
                assignment.assignment_id == mongo_data.modules[i].quiz_link
            );
            if (quiz_object) {
              ...
              if (quiz_grade > parseFloat(0)) {
                ...
                if (mongo_data.modules[i].leaderboard.quiz_early_bird == "") {
                  mongo_data.modules[
                    i
                  ].leaderboard.quiz_early_bird = studentID.toString();
                  awardBadge(24);
                  mongo.updateData(
                    courseID,
                    "modules",
                    { _id: mongo_data.modules[i]._id },
                    mongo_data.modules[i],
                    function(err, result) {}
                  );
                } else {
                  if (
                    mongo_data.modules[i].leaderboard.quiz_early_bird ==
                    studentID.toString()
                  ) {
                    awardBadge(24);
                  }
                ...
                if (
                  mongo_data.modules[i].leaderboard.quiz_leaderboard.find(
                    placement => placement.student_id == studentID
                  )
                ) {
                  //user is already on leaderboard
                  ...
                  user_index = mongo_data.modules[
                    i
                  ].leaderboard.quiz_leaderboard.findIndex(
                    placement => placement.student_id == studentID
                  );
                  mongo_data.modules[i].leaderboard.quiz_leaderboard[
                    user_index
                  ] = {
                    student_id: studentID.toString(),
                    score: quiz_grade
                  ...
                  mongo_data.modules[
                    i
                  ].leaderboard.quiz_leaderboard = mongo_data.modules[
                    i
                  ].leaderboard.quiz_leaderboard.sort(sortLeaderboardScores);
                  if (
                    mongo_data.modules[
                      i
                    ].leaderboard.quiz_leaderboard.findIndex(
                  ...
                  // Process leaderboard if not full - add user automatically
                  if (
                    mongo_data.modules[i].leaderboard.quiz_leaderboard.length <
                    10
                  ) {
                    ...
                    if (
                      ...
                      mongo_data.modules[
                        ...
                        i
                      ].leaderboard.quiz_leaderboard.sort(
                        sortLeaderboardScores
                      );
                      if (
                        mongo_data.modules[
            ...
            var reflection_object = data.find(
              assignment =>
                assignment.assignment_id ==
                mongo_data.modules[i].reflection_link
            );
            if (reflection_object) {
              ...
              if (reflection_grade == parseFloat(100)) {
                ...
                if (
                  mongo_data.modules[i].leaderboard.reflection_early_bird == ""
                ) {
                  mongo_data.modules[
                    i
                  ].leaderboard.reflection_early_bird = studentID.toString();
                  awardBadge(25);
                  mongo.updateData(
                    courseID,
                    "modules",
...
function updateCanvas(studentID, courseID, totalPoints, badges, callback) {
  // Update Canvas custom points column
...
function getIndScoreAndBadges(studentID, courseID, callback) {
  // Get score and badge info for user
  computeScoreAndBadges(studentID, courseID, function(
    err,
    totalPoints,
    badges
  ) {
...
function getStudentProgress(studentID, courseID, callback) {
  // Get student progress for quizzes and tests (checkboxes)
  mongo.getAllData(courseID, function(mongo_data) {
    getRequest(assignment_user_url(studentID, courseID), studentID, function(
      err,
      user_assigments
          ...
          if (module_object.open == "true") {
            ...
            if (practice_object) {
              moduleProgress[i].practice_progress =
                parseFloat(practice_object.grade) >=
                parseFloat(module_object.practice_cutoff);
            } else {
              moduleProgress[i].practice_progress = false;
            }

            //quiz progress
            var quiz_object = user_assigments.find(
              assignment => assignment.assignment_id == module_object.quiz_link
            );
            if (quiz_object) {
              moduleProgress[i].quiz_progress =
                parseFloat(quiz_object.grade) >=
                parseFloat(module_object.quiz_cutoff);
            } else {
...
function getLeaderboardScores(studentID, courseID, callback) {
  // get all leaderboard scores

  function mergeLeaderboardArrays(groupNames, scores) {
    //merge name and score arrays for leaderboard
    var combinedArray = [];
  ...
  asyncStuff.waterfall([getSections, getTotalScores], function(
    err,
    scores,
    groupNames,
    studentIndex
    ...
    callback(
      err,
      mergeLeaderboardArrays(groupNames, scores).sort(compare),
      mergeLeaderboardArrays(groupNames, scores)[parseInt(studentIndex)]
    );
  });
  ...
  function getSections(callback) {
    ...
    getAdminRequest(sections_url(courseID), function(err, data) {
      ...
      if (data.length < 1 || config.disableLeaderboard) {
        //disable leaderboard until sections are made
        callback(null, [], [], 0);
      } else {
    ...
    get_update_url(courseID, function(update_url) {
      getAdminRequest(update_url, function(err, pointsInfo) {
        function getPointValue(studentID) {
          try {
            return parseInt(
              pointsInfo.find(studentInfo => studentInfo.user_id == studentID)
                .content
            );
          } catch (e) {
            return 0;
        ...
        var studentPoints = studentIdsArrays.map(studentIds =>
          studentIds
            .map(studentId => getPointValue(studentId))
            .reduce((a, b) => a + b, 0)
        );
...
function getNextDailyYalie(courseID, callback) {
  getAdminRequest(daily_yalie_url(courseID), function(err, dailies_data) {
    ...
    dailies_data.forEach(function(daily) {
      if (
        new Date(daily.due_at) >= new Date() &&
        new Date(daily.due_at) < closest
      ) {
        closest = daily;
      }
    });
...
function computeScoreAndBadges_masquerade(studentID, courseID, callback) {
  // Return score and badges
  mongo.getAllData(courseID, function(mongo_data) {
    var badges = mongo_data.badges;
    var totalPoints = 0;
    ...
    getAdminRequest(assignment_user_url(studentID, courseID), function(
      err,
      data
    ) {
      if (err) {
...
function updateCanvas_masquerade(
  studentID,
  courseID,
  totalPoints,
  badges,
...
function getIndScoreAndBadges_masquerade(studentID, courseID, callback) {
  // Get score and badge info for user
  computeScoreAndBadges_masquerade(studentID, courseID, function(
    err,
    totalPoints,
    badges
  ) {
...
function getStudentProgress_masquerade(studentID, courseID, callback) {
  // Get student progress for quizzes and tests (checkboxes)
  mongo.getAllData(courseID, function(mongo_data) {
    getAdminRequest(assignment_user_url(studentID, courseID), function(
      err,
...
function getLeaderboardScores_masquerade(studentID, courseID, callback) {
  // get all leaderboard scores
